...
OppositionResultEnum = Literal[
    "Opposition likely to succeed", "Opposition may partially succeed", "Opposition likely to fail"
]
...
class GoodService(BaseModel):
    ...
    nice_class: int = Field(
        ..., ge=1, le=45, description="The Nice Classification class number (1-45)"
    )
...
class ConceptualSimilarityScore(BaseModel):
    ...
    score: Annotated[float, Field(ge=0.0, le=1.0)] = Field(
        ..., description="Conceptual similarity score"
    )
...
class MarkSimilarityOutput(BaseModel):
    ...
    overall: EnumStr = Field(
        ..., description="Overall similarity category considering all dimensions"
    )
...
class GoodServiceLikelihoodOutput(BaseModel):
    ...
    are_competitive: bool = Field(
        ..., description="Whether the goods/services compete in the marketplace"
    )
    are_complementary: bool = Field(
        ..., description="Whether the goods/services are complementary or used together"
    )
    similarity_score: Annotated[float, Field(ge=0.0, le=1.0)] = Field(
        ..., description="Similarity score between the goods/services"
    )
    likelihood_of_confusion: bool = Field(
        ...,
        description="Whether there is a likelihood of confusion for this G/S pair considering mark similarity",
    )
    confusion_type: ConfusionTypeEnum | None = Field(
        None, description="Type of confusion (null if no likelihood)"
    )
...
class OppositionOutcome(BaseModel):
    ...
    reasoning: str = Field(
        ..., description="Detailed reasoning supporting the predicted outcome and confidence"
    )
...
class CasePredictionResult(BaseModel):
    ...
    mark_comparison: MarkSimilarityOutput = Field(
        ..., description="Detailed mark similarity assessment"
    )
    goods_services_likelihoods: list[GoodServiceLikelihoodOutput] = Field(
        ...,
        description="List of detailed likelihood assessments for each applicant vs. opponent good/service pair",
    )
    opposition_outcome: OppositionOutcome = Field(
        ..., description="Structured prediction of the opposition outcome including reasoning"
    )
...
class GsSimilarityRequest(BaseModel):
    ...
    mark_similarity: MarkSimilarityOutput = Field(
        ..., description="Mark similarity assessment from /mark_similarity endpoint"
    )
...
class BatchGsSimilarityRequest(BaseModel):
    ...
    applicant_goods: list[GoodService] = Field(
        ..., min_length=1, description="List of the applicant's goods/services"
    )
    opponent_goods: list[GoodService] = Field(
        ..., min_length=1, description="List of the opponent's goods/services"
    )
    mark_similarity: MarkSimilarityOutput = Field(
        ..., description="Mark similarity assessment from /mark_similarity endpoint"
    )
...
class CasePredictionRequest(BaseModel):
    ...
    mark_similarity: MarkSimilarityOutput = Field(
        ..., description="The mark similarity assessment from /mark_similarity endpoint"
    )
    goods_services_likelihoods: list[GoodServiceLikelihoodOutput] = Field(
        ..., min_length=1, description="The G/S likelihood assessments from /gs_similarity endpoint"
    )
